...
import { Fragment, Suspense, createContext, lazy, useEffect, useReducer, useState } from 'react';
...
const App = () => {
  ...
  const [tolibData, setTolibData] = useState([]);

  const getData  = async() => {
    const data = await axios.get("https://random-data-api.com/api/stripe/random_stripe");
    setTolibData(tolibData);
  };

  console.log(tolibData);
